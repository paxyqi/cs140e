...
impl MiniUart {
    ...
    pub fn new() -> MiniUart {
        ...
        registers.AUX_MU_LCR_REG.write(0x3); // 8-bit data size
        // baudrate = system_clock_freq / (8 * (baud_reg + 1))
        // system_clock_freq is 250MHz (Page 10)
        // 250MHz / 8 / 115200 - 1 = 270
        // Using 115200 for speed
        registers.AUX_MU_BAUD.write(541); // 16-bit baudrate register
        registers.AUX_MU_CNTL_REG.write(0x3); // enable TX & RX
    ...
    pub fn write_byte(&mut self, byte: u8) {
        while !self.registers.AUX_MU_LSR_REG.has_mask(LsrStatus::TxAvailable as u8) { continue }
        self.registers.AUX_MU_IO_REG.write(byte as u8);
    }
    ...
    impl io::Read for MiniUart {
        fn read(&mut self, buf: &mut [u8]) -> io::Result<usize> {
           if self.wait_for_byte().is_err(){
            Err(io::Error::new(io::ErrorKind::TimedOut,"TimedOut waiting for data"))
            }else{
