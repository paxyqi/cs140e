...
impl<'a, T: 'a> StackVec<'a, T> {
    ...
    pub fn with_len(storage: &'a mut [T], len: usize) -> StackVec<'a, T> {
        if len>storage.len(){
            panic!("len should not be greater than storage.len()")
        }
        StackVec{
            storage:storage,
            len:len,
    ...
    /// Returns true if the vector is at capacity.满负荷=at capacity
    ...
    pub fn insert(&mut self,index:usize,element:T)->Result<(),()>{
        if self.len == self.storage.len() || index > self.len(){
            ...
            for i in (index..self.len).rev(){
                self.storage[i+1]=self.storage[i].clone()//i think there is somthing wrong===ohh,there is nothing wrong because of using rev()
            }
